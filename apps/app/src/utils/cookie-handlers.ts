...
export function setCookie(name: string, value: string, options: { expires?: number; path?: string } = {}): void {
    ...
    let cookie = `karsilo.${uid}.${name}=${safe}; path=${options.path || '/'};`;
...
export function removeCookie(name: string): void {
    ...
    document.cookie = `karsilo.${uid}.${name}=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT;`;
}
